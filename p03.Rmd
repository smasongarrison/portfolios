...
final_simulate_individual_x_y <- function(r,n) {
 ...
 correlation_data <- data.frame(r = r, calculated_corr = calculated_corr)
 corr_df_name <- paste0("correlation_x_y_", r, "_", n)
 assign(corr_df_name, correlation_data, envir = .GlobalEnv)
} 
```
...
```{r now creating a function to do the above 10,000 times, message=FALSE}
simulate_repeated_x_y <- function(r, n) {
  x_y_values <- replicate(100, final_simulate_individual_x_y (r, n))
  simulate_r_ds_fromxy <- data.frame(id = 1:100, corr = corr_values, pop_r = r, n = n)
  df_name <- paste0("final_simulated_data_", r, "_", n)
  assign(df_name, simulate_r_ds_fromxy, envir = .GlobalEnv) 
}
